...
def test_analyze_arm():
    binPath = PurePath(Path(__file__).parent.parent.parent, Path('binaries/secret1/secret-arm.bin'))
    armTargetAddr = []
...
def test_analyze_ia32():
    binPath = PurePath(Path(__file__).parent.parent.parent, Path('binaries/secret1/secret-x86-32.bin'))
    armTargetAddr = []
    armTargetPath = PurePath(binPath.parent, Path('secret-dep.json'))
    with open(armTargetPath) as f:
        data = json.load(f)
...
def test_analyze_x86_64():
    binPath = PurePath(Path(__file__).parent.parent.parent, Path('binaries/secret1/secret-x86-64.bin'))
    armTargetAddr = []
    armTargetPath = PurePath(binPath.parent, Path('secret-dep.json'))
    with open(armTargetPath) as f:
        data = json.load(f)
